...
However, before validating samples on real hardware, we need to understand how well can existing techiniques predict on real hardware, so we implemented the state-of-the-art LoopPoint methodology in gem5 simulator to evaluate the sampling methodlogy with NPB benchmark on the gem5 simulator in full-system simulation, which was not done before.
