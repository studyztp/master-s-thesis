\subsection{Sample Validating}\label{sec:verification}

Nugget addresses the limitations of using simulation to validate selected samples (Section~\ref{sec:validating_the_selected_samples_is_infeasible}) by generating \textit{nuggets} that can run directly on both real hardware and simulation.
...
Validating the same samples on multiple platforms enhances confidence in their \textit{representativeness and robustness}. 
As shown later in Section~\ref{sec:model_performance_evaluation}, we found that having \textit{consistent prediction error across platforms} is a stronger indicator of sample quality than achieving low error on a single platform.
