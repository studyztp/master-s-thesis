...
% This is time and human resource intensive.
...
Many techniques (i.e. SimPoint~\cite{simpoint}, BarrierPoint~\cite{barrier_point}, and LoopPoint~\cite{looppoint}) follow this approach.
% These techniques improve over prior techniques by being \emph{automatic} and not requiring domain experts.

In this paper, we focus on targeted sampling techniques, which use profiling to automatically select samples.
...
\revision{
To address these limitations, we present a framework called \emph{Nugget} for agile targeted sampling development.
}
...
\revision{
Therefore, Nugget is designed to be flexible and extensible, allowing researchers to create new sample selection algorithms that can be tailored to their targeted workload and input without worrying about the current limitations of targeted sampling techniques.
}
Additionally, using Nugget for sample selection methodologies, because of its nature of cross-platform compatibility and its ability to run on real hardware, enables richer insights into the selected samples and the systems that run the samples.
...
\begin{itemize}
    ...
    \item \textbf{An LLVM IR level unit of work}, enabling sample selection algorithms to create and locate samples for a program in a cross-platform manner, agnostic of machine level instructions, such as architectural specific instructions and optimizations.
...
% In this paper, we present the motivations behind Nugget and review prior work in Section~\ref{sec:motivation_and_prior_work}.
% We then describe Nugget in Section~\ref{sec:the_nugget_framework}, covering its preparation, interval analysis, sample creation, and validation.
% In Section~\ref{sec:evaluation}, we evaluate the framework against its goals, examining the efficiency of interval analysis, the automatic generation of cross-binary samples, and the validation of selected samples.
% Section~\ref{sec:case_study} illustrates two use cases of Nugget: using nuggets on real hardware to validate the effectiveness of the sample set, and using \emph{nuggets} as microbenchmarks to verify simulation models.
