...
With these eight systems, we can draw two conclusions.
...
(2) We find that Random and basic-block vector approaches to interval selection are not microarchitecturally independent.
...
\begin{table}[!t]
  ...
  \begin{tabular}{|c|c|c|c|c|}
    ...
    Config 1
      ...
      & \makecell[l]{DDR4 2400~MHz\\38.4~GiB/s} \\ \hline
  ...
  \caption{gem5 Simulation Model Configuration. The large, small, and base out-of-order cores are internal models that target different performance characteristics.}
...
Each point is the absolute value of the error of one pair of configurations' (in simulation) or pair of hardware systems' speedup compared to the true speedup of those two systems determined by running the entire workload.
...
(1) systems with different ISAs but small microarchitecture differences,
...
In this experiment, the ISA has a noticeable impact on the binaries: for instance, the Arm binaries have vectorization enabled, whereas the x86 binaries do not.
...
The Random-selected samples show a similar pattern for both ``ft'' and ``is''.
