...
Nugget\footnote{The Nugget framework is open source and publicly available at \url{https://github.com/studyztp/Nugget-LLVM-passes}.} is designed to address the limitations of the prior sampling techniques, specifically focusing on 1) the high cost of finding samples, 2) the coupling of the samples to a specific executable binary, and 3) the infeasibility of validating the selected samples.
...
We begin by defining program progress and intervals, which ensures that samples remain independent of the binary, as described in Section~\ref{sec:unit_of_work}.
