...
Therefore, it takes approximately 250 days of wall-clock time to analyze an average SPECspeed benchmark with reference input size.
...
Most methodologies focus on predicting metrics that summarize the entire hardware performance, such as instructions per cycle (IPC) or total runtime.

To evaluate the accuracy of a set of samples selected by a sampling methodology, we compare the predicted performance metrics against ground truth measurements. 
First, we measure the target performance metric from a full execution of the program on the experimental system, which serves as the ground truth. 
Next, we measure the same metric for the selected samples and apply statistical processing to estimate the full-program value. 
Finally, we compare the predicted value to the ground truth to quantify the accuracy of the selected samples.
...
For example, empirical studies (e.g., MAV~\cite{MAV}) report outliers where SimPoint fails to capture distinct memory access behavior while SimPoint reports an average 3\% IPC error with SPEC~CPU2000~\cite{spec2000} programs~\cite{wavelet-based}. 
Hence, representative intervals should be validated.
However, the only way to validate them is to run the entire workload.

Prior sampling techniques are typically evaluated on small benchmarks or truncated runs because their validation depends on simulation~\cite{wavelet-based,taskpoint,simpoint,looppoint,smarts,fsa,cross_binary_simpoints,pacsim,barrier_point,simpoint-plus,parrot}.
...
For example, LoopPoint is validated with SPEC~CPU2017 benchmarks using train-size input with Sniper~\cite{sniper}.  
FSA~\cite{fsa} is validated with the first 30~billion instructions of the SPEC~CPU2006~\cite{spec2006} benchmarks with gem5~\cite{11_gem5,20_gem5}.
...
Evidence suggests that validation results from smaller benchmarks or partial executions do not generalize to larger benchmarks and realistic workloads~\cite{characterizing_and_comparing_prevailing_simulation_techniques,hot-region-spec2017,simpoint,spec2017-workload-characterization,wavelet-based}.
